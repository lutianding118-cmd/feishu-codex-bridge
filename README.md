...
- 新增直连安静模式：普通飞书消息直接进入 Codex，只返回最终回复；同一聊天忙时只发送一条排队提示。
- 新增文件收发：支持 `/file <路径>`、`/发文件 <路径>` 和自然语言发送本机文件到飞书；飞书发来的图片/文件会保存到当前工作区 `.feishu-inbox\YYYY-MM-DD\` 并交给 Codex 处理。
- 优化飞书 App 查看体验：图片按图片消息发送，文本/Markdown 分段发送为可阅读文本，PDF/Office/音视频按飞书文件消息发送；压缩包会提示改发可预览的具体文件。
- 新增隐藏健康检测：每 5 分钟检查桥接服务、Codex、3457 端口、飞书长连接和飞书 token，异常时先重启，仍异常时限频触发 Codex 检测修复。
- 一键安装器新增计划任务：自动创建开机隐藏启动任务和隐藏健康检测任务，避免弹出黑窗口。
